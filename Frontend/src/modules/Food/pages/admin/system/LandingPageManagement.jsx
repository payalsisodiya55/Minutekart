        ...
        {activeTab === 'homepage-video' && (
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mt-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="bg-green-600 hover:bg-green-700 text-white flex items-center gap-1.5 py-2 px-4 h-10 text-sm"
                  >
                  ...
                  <Button
                    ...
                    className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 h-10 text-sm"
                  >
              ...
              {success && success.includes('Settings saved') && (
                <div className="mb-6 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg flex items-center gap-2 animate-bounce">
              ...
              {settingsLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {settings.headerImages.map((imageUrl, index) => {
                    const item = typeof imageUrl === 'object' && imageUrl !== null 
                      ? imageUrl 
                      : { url: imageUrl, showOrderNow: false, orderNowRoute: '' };

                    const handleToggleOrderNow = (e) => {
                      const checked = e.target.checked;
                      setSettings(prev => {
                        const newImages = [...prev.headerImages];
                        newImages[index] = {
                          ...item,
                          showOrderNow: checked,
                          orderNowRoute: checked ? (item.orderNowRoute || '/food/user/under-250') : ''
                        };
                        return { ...prev, headerImages: newImages };
                      });
                    };
                    ...
                    return (
                      <div key={index} className="border border-slate-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow bg-white flex flex-col justify-between">
                        <div>
                          <div className="relative aspect-video bg-slate-100">
                            <img src={item.url} alt={`Header Image ${index + 1}`} className="w-full h-full object-cover" />
                            <div className="absolute top-2 left-2">
                          ...
                          <div className="p-4 space-y-4">
                            <label className="flex items-center gap-2.5 cursor-pointer select-none">
                              <input 
                                type="checkbox" 
                                checked={!!item.showOrderNow}
                                onChange={handleToggleOrderNow}
                                className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer"
                              />
                              <span className="text-sm font-semibold text-slate-800">Show "ORDER NOW" Button</span>
                            </label>

                            {item.showOrderNow && (
                              <div className="space-y-2">
                                <Label className="text-xs font-bold text-slate-600">Target Page / Route</Label>
                                <select 
                                  value={item.orderNowRoute || ''}
                                  onChange={(e) => handleRouteChange(e.target.value)}
                                  className="w-full text-sm border border-slate-200 rounded-md p-2 bg-white text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 cursor-pointer"
                                >
                                  <option value="">Select page...</option>
                                  <option value="/food/user/under-250">Under 250 Section</option>
                                  <option value="/food/user/gourmet">Gourmet Section</option>
                                  <option value="/food/user/dining">Dining Section</option>
                                  <option value="/food/user/offers">Offers & Coupons</option>
                                  <option value="/food/user/wallet">User Wallet</option>
                                  <option value="/food/user/notifications">Notifications</option>
                                  <option value="/cart">Shopping Cart</option>
                                  <option value="/food/user/help">Help & Support</option>
                                </select>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="p-3 bg-slate-50 flex justify-between items-center border-t border-slate-100">
                          <span className="text-xs text-slate-400">Configure slide settings</span>
                          <Button
                            type="button"
                            variant="outline"
